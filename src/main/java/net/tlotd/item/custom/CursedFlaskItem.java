...
public class CursedFlaskItem extends Item {
    public CursedFlaskItem(Settings settings) { super(settings); }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(!context.getWorld().isClient()) {
            ...
            if (block.isOf(ModBlocks.WITCHING_TABLE) && (block.get(SOUL_CHARGES)+block.get(CURSED_SOUL_CHARGES)+block.get(ABYSSAL_SOUL_CHARGES))<3) {
                int charges = block.get(CURSED_SOUL_CHARGES);
                context.getWorld().setBlockState(positionClicked, block.with(CURSED_SOUL_CHARGES, charges+1), 2);
