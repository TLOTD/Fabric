    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(!context.getWorld().isClient()) {
            ...
            if ((state.isOf(Blocks.DIRT) || state.isOf(Blocks.GRASS_BLOCK)) && context.getWorld().getBlockState(positionClicked.up()).isOf(Blocks.AIR)) {
                ...
                if (miningLevel >= blockLevel && (state == ModBlocks.ALIEN_CONTROL_PANEL.getStateWithProperties(state).with(HARVESTED,false) || state.isIn(ModTags.Blocks.EXTRACTABLE_BLOCKS) || (ModConfigs.EXTRACTION_ORE_EXPERIMENTAL_COMPAT && name.contains("_ore")))) {
