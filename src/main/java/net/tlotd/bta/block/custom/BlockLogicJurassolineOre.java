...
public class BlockLogicJurassolineOre extends BlockLogic {
	public static final Int2IntArrayMap VARIANT_MAP = new Int2IntArrayMap();

	public BlockLogicJurassolineOre(Block<?> block, Block<?> parentBlock, Material material) {
	...
	public ItemStack[] getBreakResult(@NotNull World world, EnumDropCause dropCause, int meta, TileEntity tileEntity) {
		return switch (dropCause) {
			...
			case EXPLOSION, PROPER_TOOL, PISTON_CRUSH -> new ItemStack[]{new ItemStack(ModItems.JURASSOLINE_CRYSTAL)};
			default -> null;
		};
	}
}
