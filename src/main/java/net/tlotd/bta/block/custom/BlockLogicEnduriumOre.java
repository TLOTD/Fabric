...
public class BlockLogicEnduriumOre extends BlockLogic {

	public static final Int2IntArrayMap VARIANT_MAP = new Int2IntArrayMap();

	public BlockLogicEnduriumOre(Block<?> block, Block<?> parentBlock, Material material) {
	...
	public ItemStack[] getBreakResult(@NotNull World world, EnumDropCause dropCause, int meta, TileEntity tileEntity) {
		return switch (dropCause) {
			...
			case EXPLOSION, PROPER_TOOL, PISTON_CRUSH -> new ItemStack[]{new ItemStack(ModItems.ENDURIUM_CRYSTAL)};
			default -> null;
		};
	}
}
