...
public class SignalTransmitterAntennaBlock extends Block {
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if (state.isOf(ModBlocks.MITHRIL_SIGNAL_TRANSMITTER_ANTENNA)) {
        ...
        if (random.nextInt(5) == 1) {
            for (Direction direction : Direction.values()) {
                ...
                if (!world.getBlockState(blockPos).isOpaqueFullCube(world, blockPos)) {
                    ...
                    double e = axis == Direction.Axis.X ? 0.5F + 0.2625F * direction.getOffsetX() : random.nextFloat();
                    double f = axis == Direction.Axis.Y ? 0.5F + 0.2625F * direction.getOffsetY() : random.nextFloat();
                    double g = axis == Direction.Axis.Z ? 0.5F + 0.2625F * direction.getOffsetZ() : random.nextFloat();
