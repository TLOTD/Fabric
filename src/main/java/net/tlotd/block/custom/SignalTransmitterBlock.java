        ...
        if (!world.isClient) {
            if (world.getBlockState(pos.up()) == ModBlocks.SIGNAL_TRANSMITTER_ANTENNA.getDefaultState() && world.getBlockState(pos.up(2)) == Blocks.LIGHTNING_ROD.getDefaultState()) {
                ...
                if (player.getMainHandStack().isIn(ModTags.Items.VHS_CASSETTES) || player.getMainHandStack().isIn(ModTags.Items.MUSIC_DISCS)) {
                    ...
                    if(!world.getBlockState(zero).getBlock().equals(ModBlocks.BEDROCK) || !world.getBlockState(zero).get(CHANNEL_1)&&!world.getBlockState(zero).get(CHANNEL_2)&&!world.getBlockState(zero).get(CHANNEL_3)&&!world.getBlockState(zero).get(CHANNEL_4)&&!world.getBlockState(zero).get(CHANNEL_5)&&!world.getBlockState(zero).get(CHANNEL_6)&&!world.getBlockState(zero).get(CHANNEL_7)&&!world.getBlockState(zero).get(CHANNEL_8)&&!world.getBlockState(zero).get(CHANNEL_9)&&!world.getBlockState(zero).get(FREQUENCY_1)&&!world.getBlockState(zero).get(FREQUENCY_2)&&!world.getBlockState(zero).get(FREQUENCY_3)&&!world.getBlockState(zero).get(FREQUENCY_4)){
